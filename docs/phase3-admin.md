...
installable onEdit triggerは「申請一覧」のA列と交差する編集だけを対象にします。複数行貼付ではevent valueを使わず、対象範囲を上から順に処理します。自動対象は`A=○`かつ`M=未処理`だけです。

対象範囲全体で`ScriptLock.tryLock(30000)`を1回だけ取得し、Spreadsheet・申請一覧・kkjの参照を再利用します。複数行を同時に○にしても行ごとにlockを取り合いません。lock競合時は申請行をエラーへ変更せず`未処理`のまま延期します。1行が入力不備などで失敗しても後続行を処理します。
...
level blockの分断や順序逆転だけであれば、○反映時に既存Spreadsheetの`sortMainSheetByLevel_`と同じ順序へ自動整列します。`10- → 10 → 10+`とし、特殊表記を数値難易度の後、`?`を必ず最末尾にする既存規則を変更しません。同一level内の行順は維持し、連続する行をblock単位で移動します。内部空行、未知level、不正MD5は`TABLE_ORDER_INVALID`、MD5重複は`CHART_DUPLICATED`で停止し、データ内容を推測して修正しません。
